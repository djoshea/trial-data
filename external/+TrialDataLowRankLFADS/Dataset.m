classdef Dataset < LowRankLFADS.Dataset
    ...
    methods
        function ds = Dataset(collection, tdPathOrShortName, td) 
            ds = ds@LowRankLFADS.Dataset(collection, tdPathOrShortName);
        ...
        function td = loadData(ds, reload)
            ...
            if isempty(ds.trialData) || reload
                ...
                td = ds.modifyTrialDataPostLoad(td);
        ...
        function td = modifyTrialDataPostLoad(ds, td)
            % can override in subclasses to perform time consuming data cleaning
        end
    end
end
