classdef AlignDescriptor 
    ...
    methods % Dependent properties
        ...
        function tf = get.isFullTrial(ad)
            tf = ad.isStartTrialStart && ad.isStopTrialEnd;
        end
        
        function tf = get.isStartTrialStart(ad)
            tf = strcmp(ad.startEvent, 'TrialStart') && ad.startOffset == 0;
        end
        
        function tf = get.isStopTrialEnd(ad)
            tf = strcmp(ad.stopEvent, 'TrialEnd') && ad.stopOffset == 0;
        end
...
%         end
    ...
    methods % String parsing and building, disp() 
        ...
        function printDescription(ad, varargin)
            ...
            if ~ad.nameDefault
                if p.Results.active
                    TrialData.cprintf('inline', '{yellow}%s: {white}%s {red}(active)\n', class(ad), ad.name);
                else
                    TrialData.cprintf('inline', '{yellow}%s: {white}%s\n', class(ad), ad.name);
                end
            else
                % name will just match desc, so don't print it twice
                if p.Results.active
            ...
            TrialData.cprintf('inline', '  {green}minimum duration {white}%d\n', ad.minDuration);
            if ad.roundTimes
                TrialData.cprintf('inline', '  {green}round times with min delta {white}%g\n', ad.minTimeDelta);
            end
            if ad.padPre ~= 0 || ad.padPost ~= 0
                TrialData.cprintf('inline', '  {green}pad %g pre, %g post\n', ad.padPre, ad.padPost);
            end
        end
