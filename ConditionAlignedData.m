classdef ConditionAlignedData 
% This class efficiently represents a dataset with nChannels of data stored
% with nAlign alignments over nTrials of data. Each trial has associated attributes
% (via ConditionInfo) and alignment events (via AlignInfo).

    properties 
        % whatever you'd like
        meta

        % nTrials x nAlign x nChannels data cell or matrix
        data = {};

        % nTrials x nAlign x nChannels cell for optional time vectors when storing timeseries in data
        time = {};
        
        % nAlign cellvec for each alignment
        alignInfoCell
        
        % associated conditionInfo
        conditionInfo
    end
    ...
    methods % Constructor 
        function cad = ConditionAlignedData()
    ...
    methods % Filtering bases, conditions
        function c = selectAlongDim(c, dim, idx)
            c.warnIfNoArgOut(nargout);
            c.data = TensorUtils.selectAlongDim(c.data, dim, idx); 
            c.time = TensorUtils.selectAlongDim(c.time, dim, idx); 
            
            if dim == DIM_TRIAL
                td = td.createCopyConditionInfo();
                td.conditionInfo.selectTrials(idx);
            end

            if dim == DIM_ALIGN 
                c.alignInfoCell = c.alignInfoCell(idx);
            end
        end
...
%         
    ...
    methods(Static)
        function pset = fromRawData(dataCell)    
