...
%   based off Matlab's finddelay
    ...
    if nargout > 1
        aligned = TensorUtils.undoReshapeByConcatenatingDims(aligned, {timeDim, channelDims, groupDims, loopOverDims}, szDataOrig);
    end
    
    % then reshape delays to match that of data
    delays = TensorUtils.undoReshapeByConcatenatingDims(delays, {timeDim, channelDims, groupDims, loopOverDims}, szDelays);
end
