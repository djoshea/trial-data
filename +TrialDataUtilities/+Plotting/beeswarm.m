function x = beeswarm_get_locations(x,y,varargin)
% Note: This function is a lightly modified version of Ian Stevenson's code at https://github.com/ihstevenson/beeswarm/
% 
...
if isfinite(p.Results.dot_size)
    ...
    asp_rat = pasp_rat(1)/pasp_rat(2); % width / height
    
    % pix-scale
    pa = getpixelposition(gca);
    as = pa(3:4) * p.Results.aspect_ratio_gain; % width and height of panel in pixels
...
for i=1:length(ux)
    ...
    if length(fid)>1
        ...
        for j=1:length(fid)
            if strcmpi(p.Results.sort_style,'hex')
                xi = linspace(-chanwid/2*rmult,chanwid/2*rmult,nxloc*rmult+(mod(nxloc*rmult,2)==0))'+ux(i);
            ...
            if ~strcmpi(p.Results.sort_style,'hex') && ~strcmpi(p.Results.sort_style,'square')
                if sum(zid)>0
            ...
            if strcmpi(p.Results.sort_style,'one')
                e(xi<ux(i))=Inf;
            end
            [~,mini] = min(e);
...
if strcmpi(p.Results.sort_style,'randn')
...
if isfinite(p.Results.dot_size) && p.Results.show_plot
