function [h, x,y,z]=tubeplot(curve,r, varargin)
...
% Janus H. Wesenberg, july 2004
    ...
    p = inputParser();
    p.addParameter('nPointsAround', 8, @isscalar);
    p.addParameter('collapseThreshold', r/2, @isscalar);
    p.KeepUnmatched = true; % extra args go to surf
    p.parse(varargin{:});
    ...
    n = p.Results.nPointsAround;
    ct = p.Results.collapseThreshold;
    
    % remove trailing nans
    nanMask = any(isnan(curve), 1);
    idxStart = find(~nanMask, 1, 'first');
    idxStop = find(~nanMask, 1, 'last');
    curve = curve(:, idxStart:idxStop);
    ...
    %... and plot:
    h = surf(x,y,z);
    set(h, 'Clipping', 'off', p.Unmatched);
end
